...
async def end_poll(ctx):  # Command that lets you end polls that are stored in the data file and currently running
    ...
    msg = await ctx.send(content="Ce message s'éffacera dans 15 secondes.", embed=em, components=components)  # We send the message containing the Select and store it into a variable
    try:
        res = await client.wait_for("select_option", check=lambda i: i.author == sctx.author and i.channel == sctx.channel, timeout=15)  # We wait for the author of the commands to pick an option
    ...
    poll = polls[str(sctx.guild.id)][key]  # We get the poll dictionary
